...
# Contract enforcement will be performed at runtime inside execute_tool().
...
# Expose a public alias for the registry.  Other modules can import
# TOOL_REGISTRY to inspect or invoke available tools.  This alias
# always reflects the current allowlist filtering.
...
# Unified dispatcher for executing tools.  This wrapper supports both
# free‑form `input_text` calls and structured keyword argument calls.
def execute_tool(
    tool: str,
    ...
    **kwargs: Any,
    ...
    """
    Dispatch a tool invocation using either a free‑form text input or
    keyword arguments.  If keyword arguments are supplied, the call is
    delegated to `_execute_tool_named`.  Otherwise the free‑form
    string is passed to the tool implementation with contract
    enforcement and auditing.

    Args:
        tool: Name of the tool to execute.
        input_text: Free‑form text input for tools that accept a
            single string argument.  Ignored when keyword arguments
            are provided.
        user_id: Identifier for audit/logging.
        **kwargs: Structured arguments for tools expecting keyword
            parameters.

    Returns:
        Result from the tool invocation or a blocked/error message.
    """
    # If keyword arguments are provided, dispatch to the named executor
    if kwargs:
        return _execute_tool_named(tool, **kwargs)

    text = input_text or ""
    ctx = {"tool": tool, "user": user_id}

    # Contract enforcement: evaluate against default contracts
    try:
        gate = enforce_contract(default_contracts(), tool, {"input": text}, user_region="ID")
    except Exception:
        gate = {"ok": True}
    if not gate.get("ok", True):
        _audit({"allow": False, "reason": gate.get("reason", "contract_denied"), **ctx})
        return f"[BLOCKED] {gate.get('reason', 'contract_denied')}"
    ...
    if tool not in _REGISTRY:
        ...
        _audit({"allow": True, "invoke": tool, **ctx})
        result = _run_with_timeout(
            _REGISTRY[tool], (text,), timeout_s=_DEFAULT_TIMEOUT_S
        )
